...
// Dữ liệu câu hỏi từ file Data_CauHoiTuongTac.txt
const data = [
  {
    question: "Khi giảng viên bước vào lớp, tất cả tu sinh phải làm gì?",
    choices: [
      "Ngồi yên và chắp tay",
      "Đứng dậy, mặt hướng về giảng viên",
      "Đứng dậy, mặt hướng về tượng Phật"
    ],
    answer: 2,
    explanation: "Theo nghi lễ, khi giảng viên bước vào, tất cả tu sinh đều đứng dậy, mặt hướng về tượng Phật để bày tỏ sự tôn kính."
  },
  {
    question: "Mục đích của việc niệm 'Nam Mô Bổn Sư Thích Ca Mâu Ni Phật' ba lần là gì?",
    choices: [
      "Để bắt đầu giờ học có không khí trang nghiêm",
      "Để thể hiện lòng biết ơn và cung kính đối với đức Phật",
      "Để tạo âm thanh đồng thanh cho dễ nhớ"
    ],
    answer: 1,
    explanation: "Câu niệm Phật ba lần mang ý nghĩa thể hiện lòng biết ơn sâu sắc và sự cung kính tôn trọng đối với đức Phật."
  },
  {
    question: "Vì sao giảng viên đọc 'NAM … MÔ' trước, rồi tu sinh đọc tiếp chữ 'MÔ'?",
    choices: [
      "Để tu sinh không đọc sai câu niệm",
      "Để tránh đọc trùng nhau",
      "Để mọi người bắt nhịp đồng đều"
    ],
    answer: 2,
    explanation: "Việc giảng viên đọc 'NAM … MÔ' rồi tu sinh nối tiếp giúp mọi người cùng bắt một nhịp, đảm bảo sự đồng thanh và trang nghiêm."
  },
  {
    question: "Sau khi niệm Phật, giảng viên và tu sinh làm gì tiếp theo?",
    choices: [
      "Ngồi xuống liền",
      "Xá tượng Phật và xá hình Trưởng Lão",
      "Mở bài học ngay"
    ],
    answer: 1,
    explanation: "Sau khi niệm Phật xong, mọi người thực hiện nghi thức xá tượng Phật và xá hình Trưởng Lão như một cách bày tỏ lòng kính trọng."
  },
  {
    question: "Câu niệm Phật được giữ lại trong nghi lễ với lý do gì?",
    choices: [
      "Vì ngắn gọn, dễ nhớ",
      "Vì phù hợp với nghi lễ truyền thống",
      "Vì mang đầy đủ ý nghĩa về lòng biết ơn và tôn kính"
    ],
    answer: 2,
    explanation: "Dù nghi lễ đơn giản, nhưng câu niệm Phật vẫn được giữ lại vì mang ý nghĩa rất sâu sắc: thể hiện lòng biết ơn và tôn kính đức Phật."
  },
  {
    question: "Trong buổi học đầu tiên, giảng viên cần làm gì thêm sau phần nghi lễ?",
    choices: [
      "Bắt đầu bài học ngay",
      "Giải thích thêm về ý nghĩa nghi lễ",
      "Nhắc nhở tu sinh học thuộc nghi lễ"
    ],
    answer: 1,
    explanation: "Trong buổi đầu, giảng viên cần giải thích thêm ý nghĩa nghi lễ để tu sinh hiểu rõ và thực hành đúng."
  },
  {
    question: "Nếu tu sinh đến trễ khi nghi lễ đang diễn ra thì nên làm gì?",
    choices: [
      "Ngồi ngay vào chỗ không cần tham gia",
      "Tham gia ngay phần đang thực hiện và hòa vào nhịp chung",
      "Chờ nghi lễ xong mới vào chỗ ngồi"
    ],
    answer: 1,
    explanation: "Tu sinh đến trễ nên nhanh chóng tham gia phần đang diễn ra để thể hiện tinh thần hoà hợp và tôn trọng nghi lễ."
  }
];
...
function updateProgressBar() {
    // Tính toán tiến độ dựa trên số câu đã qua (currentQuestionIndex)
    // Và tổng số câu (data.length)
    const progress = (currentQuestionIndex / data.length) * 100;
    progressBar.style.width = `${progress}%`; // Cập nhật chiều rộng của thanh progress
    progressText.textContent = `${currentQuestionIndex}/${data.length}`; // Cập nhật văn bản tiến độ
}
...
function startTimer() {
    // Chỉ bắt đầu nếu timer được bật và chưa có timer nào đang chạy
    if (!isTimerEnabled) {
...
function displayQuestion() {
    ...
    const originalChoices = [...currentQuestion.choices];
...
function checkAnswer(isTimeout = false) {
    ...
    if (isTimeout && selectedChoice === -1) {
        // Tự động chọn một đáp án sai (hoặc không chọn gì cả, coi là sai)
        // Để đơn giản, ta sẽ coi như người dùng không chọn gì và kết quả là sai.
        // Tuy nhiên, để lưu vào questionsAttempted, ta có thể gán selectedChoice = -1
        // Và đánh dấu câu trả lời sai.
    ...
    const currentQuestion = data[currentQuestionIndex];
    const correctOriginalIndex = currentQuestion.answer;
